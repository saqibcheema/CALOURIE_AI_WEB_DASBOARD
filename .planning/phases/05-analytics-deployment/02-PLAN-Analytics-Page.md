...
# Plan 02 — Analytics Page

## Goal

Create `app/dashboard/analytics/page.tsx` with 4 stat cards (today's totals), a 4-tab metric selector, and a full labeled 7-day bar chart powered by `/api/analytics`.

---

## Tasks

### Task 2.1 — Create `app/dashboard/analytics/page.tsx`

<read_first>
- `app/dashboard/page.tsx` — see existing layout pattern (space-y-6 p-6 bg-slate-50 min-h-screen), stat cards grid, SparklineChart wrapper — replicate the same structure
- `components/StatCard.tsx` — props: title, value, icon, trend
- `components/SparklineChart.tsx` — already uses 'use client'; reference for Recharts ResponsiveContainer pattern
- `.planning/phases/05-analytics-deployment/05-UI-SPEC.md` — tab styles, bar chart config, color tokens, copy
- `.planning/phases/05-analytics-deployment/05-CONTEXT.md` — D-12 through D-16 for analytics page decisions
</read_first>

<action>
Create `app/dashboard/analytics/page.tsx` with the following structure:
...
<acceptance_criteria>
- `app/dashboard/analytics/page.tsx` exists
- File contains `'use client'`
- File contains `import { useCallback, useEffect, useState, ReactNode } from 'react'`
- File contains `import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer } from 'recharts'`
- File contains `import { StatCard } from '@/components/StatCard'`
- File contains `type MetricKey =` with all 4 metric keys
- File contains `setActiveMetric` state
- File contains `<Bar dataKey={activeMetric} fill="#0ea5e9" radius={[4, 4, 0, 0]}`
- File contains 4 tab buttons (grep: `METRICS.map`)
- File contains loading skeleton (`animate-pulse`)
- File contains error state with Retry button
- File contains `buildTrend` function with division-by-zero guard (`sevenDaysAgo === 0`)
...
```
must_haves:
  - Analytics page exists at app/dashboard/analytics/page.tsx
  - 4 stat cards with real data and trend comparison
  - Bar chart renders with 4-tab metric selector
